...
func New(cfg *conf.Global, agent *svc.Agent, session domain.Session, askUserCh chan askuser.Request) *Controller {
	return &Controller{cfg: cfg, agent: agent, session: session, askUserCh: askUserCh, historyAt: -1}
}

func (c *Controller) Attach(runtime *app.App, invalidate func(), setAskOpen func(bool)) {
	c.mu.Lock()
	c.runtime, c.invalidate, c.setAskOpen = runtime, invalidate, setAskOpen
	c.mu.Unlock()
	go c.askLoop()
}
